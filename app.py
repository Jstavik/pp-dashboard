...
from charts.imbalance import (
    ...
    balancing_strategy_ema, balancing_strategy_holt, fig_balancing_strategy,
)
...
if not show_gas:
    ...
    with tab_dash:
        ...
        st.info(
            ...
            "EMA (oranžová) dává větší váhu posledním intervalům a slouží jako proxy aktuálního stavu — "
            "do budoucnosti se prodlužuje konstantně (přerušovaná čára). "
            "Holt's predikce (růžová) přidává trend komponentu a projektuje očekávaný vývoj "
            "odchylky 30 minut dopředu (přerušovaná čára). "
            "Graf je prodloužen o 2 h za aktuální čas, aby byla predikce viditelná. "
            "Zákazníci v balancing segmentu pomáhají síti a jsou za to benefitováni."
        )
        st.subheader("⚡ Balancing strategie (EMA + Holt's predikce)")
        ...
        if not df_ceps_imbal.empty or not df_imbal.empty:
            ...
            STEP_MIN = 5
            if not _ema.empty:
                _future_idx = pd.date_range(
                    start=_ema.index[-1] + pd.Timedelta(minutes=STEP_MIN),
                    periods=HORIZON, freq=f"{STEP_MIN}min",
                )
                _ema_forecast = pd.Series(_ema.iloc[-1], index=_future_idx)
            else:
                _ema_forecast = pd.Series(dtype=float)
            _holt_smoothed, _holt_forecast = balancing_strategy_holt(
                imbal_5min, alpha=0.3, beta=0.1,
                horizon=HORIZON, step_minutes=STEP_MIN,
            )
            st.plotly_chart(
                fig_balancing_strategy(df_imbal, _ema, _signal, threshold_mw, now,
                                       ema_forecast=_ema_forecast,
                                       holt_smoothed=_holt_smoothed,
                                       holt_forecast=_holt_forecast),
                use_container_width=True, config={"displayModeBar": False},
            )
